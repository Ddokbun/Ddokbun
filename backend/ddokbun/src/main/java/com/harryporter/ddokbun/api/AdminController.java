...
public class AdminController {
    ...
    public ResponseEntity<?> getPlant(HttpServletRequest request){
        if(!userService.areYouAdmin(jwtTokenUtils.getUserSeq(request)))
    ...
    @PostMapping("/plant")
    public ResponseEntity<?> insertPlant(HttpServletRequest request, @RequestBody PlantDto plantDto){
        if(!userService.areYouAdmin(jwtTokenUtils.getUserSeq(request)))
            throw new GeneralException(ErrorCode.BAD_REQUEST,"관리자 계정이 아닙니다");
        log.info("관리자 접속 완료");
    ...
    @PutMapping("/plant")
    public ResponseEntity<?> updatePlant(HttpServletRequest request, @RequestBody PlantDto plantDto){
        if(!userService.areYouAdmin(jwtTokenUtils.getUserSeq(request)))
            throw new GeneralException(ErrorCode.BAD_REQUEST,"관리자 계정이 아닙니다");
        log.info("관리자 접속 완료");
